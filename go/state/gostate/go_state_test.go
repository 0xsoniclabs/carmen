...
func TestHashing(t *testing.T) {
	...
	for _, config := range states {
		t.Run(config.name(), func(t *testing.T) {
			...
			hash3, err := state.GetCommitment().Await()
			if err != nil {
				t.Fatalf("unable to get state hash; %v", err)
			}
			...
			hash4, err := state.GetCommitment().Await()
			if err != nil {
				t.Fatalf("unable to get state hash; %v", err)
			}
	...
	for i := 0; i < loops; i++ {
		...
		t.Run(fmt.Sprintf("operation_%d", i), func(t *testing.T) {
			...
			for i := 0; i < 2; i++ {
				...
				if _, err := db.GetCommitment().Await(); shouldFail && !errors.Is(err, injectedErr) {
					t.Errorf("operation should fail")
				}
