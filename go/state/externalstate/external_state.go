...
func (s *ExternalState) Apply(block uint64, update common.Update) ([]func(), <-chan error, error) {
	// TODO:
	...
	if result != C.kResult_Success {
		return nil, nil, fmt.Errorf("failed to apply update at block %d (error code %v)", block, result)
	}
...
func (s *ExternalState) RevertLastBlock(blockUndoList []func()) error {
	// TODO:
	return nil
}
