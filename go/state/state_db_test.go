...
func TestStateDB_ThereCanBeMultipleBulkLoadPhases(t *testing.T) {
	...
	staged := NewMockStagedBlock(ctrl)
	staged.EXPECT().Commit().Return(NewWaitHandle(nil), nil).AnyTimes()
...
func TestStateDB_EndBlock_IsRefusedAfterAnArchiveError(t *testing.T) {
	...
	staged := NewMockStagedBlock(ctrl)
	staged.EXPECT().Commit().Return(failedWrite(injectedError), nil)
...
func TestStateDbStagedBlock_Commit_ReportsMisuseWithoutCollectingIt(t *testing.T) {
	...
	staged := NewMockStagedBlock(ctrl)
	staged.EXPECT().Commit().Return(nil, misuse)
