...
func TestStateDB_StorageDataOfDestroyedAccountIsNotVisibleInNextTransactionOrBlock(t *testing.T) {
	...
	mock.EXPECT().Check().AnyTimes()
	mock.EXPECT().Apply(gomock.Any(), common.Update{}).DoAndReturn(committable).AnyTimes()
...
func TestStateDB_RollingBackSuicideRestoresBalance(t *testing.T) {
	...
	if want, got := amount.New(5), db.GetBalance(address1); want != got {
	...
	if want, got := amount.New(), db.GetBalance(address1); want != got {
		t.Errorf("unexpected balance, wanted %v, got %v", want, got)
	}
...
func TestStateDB_RollingBackSuicideRestoresValues(t *testing.T) {
	...
	mock.EXPECT().GetStorage(address1, key1).Return(val1, nil)
	mock.EXPECT().GetStorage(address1, key2).Return(val2, nil)
	mock.EXPECT().Apply(uint64(1), common.Update{}).DoAndReturn(committable)
...
func TestStateDB_RecreatingExistingAccountSetsNonceAndCodeToZeroAndPreservesBalance(t *testing.T) {
	...
	if got := db.GetBalance(address1); got != amount.New(12) {
...
func TestStateDB_RepeatedSuicide(t *testing.T) {
	...
	db.AddBalance(address1, amount.New(123))
	db.SetState(address1, key1, val1)
	...
	db.AddBalance(address1, amount.New(456))
	db.SetState(address1, key2, val2)
...
func TestStateDB_SuicideRemovesBalanceFromAccount(t *testing.T) {
	...
	if got, want := db.GetBalance(address1), amount.New(); got != want {
...
func TestStateDB_CreatedAccountsAreForgottenAtEndOfBlock(t *testing.T) {
	...
	mock.EXPECT().Apply(uint64(2), common.Update{}).DoAndReturn(committable)
...
func TestStateDB_DeletesEmptyAccountsEip161(t *testing.T) {
	...
	db.SubBalance(address1, amount.New(12))
...
func TestStateDB_NeverCreatesEmptyAccountsEip161(t *testing.T) {
	...
	db.AddBalance(address2, amount.New(12))
	db.SubBalance(address2, amount.New(12))
...
func TestStateDB_SuicidedAccountNotRecreatedBySettingBalance(t *testing.T) {
	...
	if db.GetBalance(address1) != amount.New(12) {
...
func TestStateDB_LogsAreResetAtEndOfBlock(t *testing.T) {
	...
	mock.EXPECT().Check().AnyTimes()
	mock.EXPECT().Apply(uint64(0), common.Update{}).DoAndReturn(committable)
...
func TestStateDB_NoApplyWhenErrorsHaveBeenEncountered(t *testing.T) {
	...
	state.EXPECT().Apply(uint64(1), gomock.Any()).DoAndReturn(committable)
...
// commits it right away.
// committable stands in for a state's Apply in tests that do not look at the
// staged block: every block applied can be committed, and nothing else.
func committable(uint64, common.Update) (StagedBlock, error) {
	return NewIrreversibleBlock(0, func() common.Hash { return common.Hash{} }, nil), nil
}
...
func TestStateDB_BulkLoadCloseReportsFlushIssues(t *testing.T) {
	...
	injectedError := fmt.Errorf("injected error")
	state.EXPECT().Apply(uint64(12), common.Update{}).Return(bulkStaged(ctrl), nil)
...
func TestStateDB_ThereCanBeMultipleBulkLoadPhases(t *testing.T) {
	...
	mock.EXPECT().Apply(gomock.Any(), gomock.Any()).DoAndReturn(
		func(uint64, common.Update) (StagedBlock, error) { return bulkStaged(ctrl), nil },
	).AnyTimes()
...
func TestBulkLoad_CloseResetsLocalCache(t *testing.T) {
	...
	db.AddBalance(address1, amount.New(100))
	db.SetNonce(address1, uint64(1))
	db.SetCode(address1, []byte{1})

	db.AddBalance(address2, amount.New(200))
	db.SetNonce(address2, uint64(2))
	db.SetCode(address2, []byte{2})

	db.AddBalance(address3, amount.New(300))
...
func TestStateDB_ContractIsNoLongerMarkedAsNewAfterRollback(t *testing.T) {
	...
	require.True(t, db.IsNewContract(address1),
		"contract not marked as new after CreateContract call")
...
func TestStateDB_EndBlock_ForwardsApplyDoneChannel(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		staged := NewMockStagedBlock(ctrl)
		staged.EXPECT().Commit().Return(NewWaitHandle(applyDone), nil)
...
func TestStateDB_EndBlock_CollectsMultipleSyncErrorsInIssueTracker(t *testing.T) {
	...
	// Committing reports an issue the state collected earlier and still hands out
	// the handle on the archive write, which then fails as well.
	staged := NewMockStagedBlock(ctrl)
	staged.EXPECT().Commit().Return(NewWaitHandle(applyDone), injectedError1)
...
func TestStateDB_EndBlock_ClearsUndoList(t *testing.T) {
	...
	state := NewMockState(ctrl)
	state.EXPECT().Check().AnyTimes()
	state.EXPECT().Apply(uint64(12), gomock.Any()).DoAndReturn(committable)
...
func TestStateDB_BulkLoadCloseReportsAMissingStagedBlock(t *testing.T) {
	ctrl := gomock.NewController(t)
	state := NewMockState(ctrl)
	state.EXPECT().Apply(uint64(12), common.Update{}).Return(nil, nil)

	bulk := bulkLoad{block: 12, db: createStateDBWith(state, 0, true)}
	require.ErrorContains(t, bulk.Close(), "without returning a staged block")
}

func TestStateDB_BulkLoadCloseReportsCommitIssues(t *testing.T) {
	ctrl := gomock.NewController(t)
	state := NewMockState(ctrl)
	injectedError := fmt.Errorf("injected error")
	staged := NewMockStagedBlock(ctrl)
	staged.EXPECT().Commit().Return(nil, injectedError)
	state.EXPECT().Apply(uint64(12), common.Update{}).Return(staged, nil)

	bulk := bulkLoad{block: 12, db: createStateDBWith(state, 0, true)}
	require.ErrorIs(t, bulk.Close(), injectedError)
}
