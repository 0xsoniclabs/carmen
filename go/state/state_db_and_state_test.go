...
func TestCarmen_RevertBackStorage(t *testing.T) {
	require := require.New(t)
	for _, config := range initStates() {
		if config.config.Schema != 5 || strings.Contains(config.config.String(), "flat") || config.config.Archive != state.NoArchive {
			continue
		...
		t.Run(config.name(), func(t *testing.T) {
			...
			initialHash := db.GetHash()
			require.Equal(amount.New(10), db.GetBalance(address1))
			require.Equal(val1, db.GetState(address1, key1))
			require.Equal(val2, db.GetState(address1, key2))
			require.Equal(val3, db.GetState(address1, key3))

			db.BeginBlock()
			db.BeginTransaction()
			db.Suicide(address1)
			db.EndTransaction()
			endBlockRes := db.EndBlock(2)

			require.NotEqual(initialHash, db.GetHash())
			require.Equal(common.Value{}, db.GetState(address1, key1))
			require.Equal(common.Value{}, db.GetState(address1, key2))
			require.Equal(common.Value{}, db.GetState(address1, key3))

			require.NoError(db.RevertLastBlock(endBlockRes.UndoList))
			require.Equal(initialHash, db.GetHash())
			require.Equal(amount.New(10), db.GetBalance(address1))
			require.Equal(val1, db.GetState(address1, key1))
			require.Equal(val2, db.GetState(address1, key2))
