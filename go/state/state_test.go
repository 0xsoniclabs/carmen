...
func TestCodeCanBeUpdated(t *testing.T) {
	testEachConfiguration(t, func(t *testing.T, config *namedStateConfig, s state.State) {
		...
		if _, _, err := s.Apply(0, common.Update{
			CreatedAccounts: []common.Address{address1},
		}); err != nil {
		...
		if _, _, err := s.Apply(1, common.Update{Codes: []common.CodeUpdate{{Account: address1, Code: code1}}}); err != nil {
		...
		if _, _, err := s.Apply(2, common.Update{Codes: []common.CodeUpdate{{Account: address1, Code: code2}}}); err != nil {
			t.Fatalf("failed to update code: %v", err)
		}
		code, err = s.GetCode(address1)
...
func TestDeleteNotExistingAccount(t *testing.T) {
	testEachConfiguration(t, func(t *testing.T, config *namedStateConfig, s state.State) {
		...
		if _, _, err := s.Apply(0, common.Update{CreatedAccounts: []common.Address{address1}}); err != nil {
			t.Fatalf("Error: %s", err)
		}
		if _, _, err := s.Apply(1, common.Update{DeletedAccounts: []common.Address{address2}}); err != nil { // deleting never-existed account
			t.Fatalf("Error: %s", err)
		}
...
func TestCreatingAccountClearsStorage(t *testing.T) {
	testEachConfiguration(t, func(t *testing.T, config *namedStateConfig, s state.State) {
		...
		if _, _, err = s.Apply(1, common.Update{Slots: []common.SlotUpdate{{Account: address1, Key: key1, Value: val1}}}); err != nil {
		...
		if _, _, err := s.Apply(2, common.Update{CreatedAccounts: []common.Address{address1}}); err != nil {
			t.Fatalf("Error: %s", err)
		}
...
func TestDeletingAccountsClearsStorage(t *testing.T) {
	testEachConfiguration(t, func(t *testing.T, config *namedStateConfig, s state.State) {
		...
		if _, _, err := s.Apply(2, common.Update{DeletedAccounts: []common.Address{address1}}); err != nil {
			t.Fatalf("Error: %s", err)
		}
...
func TestArchive(t *testing.T) {
	for _, config := range initStates() {
		...
		t.Run(config.name(), func(t *testing.T) {
			...
			if _, _, err := s.Apply(1, common.Update{
				Balances: []common.BalanceUpdate{
...
func TestPersistentState(t *testing.T) {
	for _, config := range initStates() {
		...
		t.Run(config.name(), func(t *testing.T) {
			...
			if _, _, err := s.Apply(0, update); err != nil {
...
func TestHasEmptyStorage_S3_Always_Returns_True(t *testing.T) {
	...
	for _, config := range initStates() {
		...
		for i, update := range updates {
			if _, _, err = st.Apply(uint64(i), update); err != nil {
