...
func checkStateDB(t *testing.T, expected *stateDB, actual *stateDB, mock *MockState, defaultAccount common.Address) error {
	...
	for addr, account := range actual.accounts {
		...
		if exists && reflect.DeepEqual(account, value) {
			continue
		}
		if account.current != account.original {
	...
	for addr, balance := range actual.balances {
		...
		if *balance.original != balance.current && balance.current != defaultBalance {
...
func Test_checkStateDB_checksStateDBRevertedFieldsAndPostconditions(t *testing.T) {
	...
	actual.balances[newAddr] = &balanceValue{current: mockBalance}
	actual.balances[newAddr].original = &actual.balances[newAddr].current
