...
func (c *headBlockContext) Commit() error {
	...
	// this API is kept, so it is promoted into the archive as well. Both happen under
	// exclusive (write) access to the head state, released before the head moves on.
	err := func() error {
