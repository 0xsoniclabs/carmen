...
func (u *Update) String() string {
	...
	// Make sorted copies of each group so the output is deterministic
	// regardless of the order in which updates were appended.
	deletedAccounts := append([]Address(nil), u.DeletedAccounts...)
	sort.Slice(deletedAccounts, func(i, j int) bool {
		return accountLess(&deletedAccounts[i], &deletedAccounts[j])
	})
	createdAccounts := append([]Address(nil), u.CreatedAccounts...)
	sort.Slice(createdAccounts, func(i, j int) bool {
		return accountLess(&createdAccounts[i], &createdAccounts[j])
	})
	balances := append([]BalanceUpdate(nil), u.Balances...)
	sort.Slice(balances, func(i, j int) bool {
		return balanceLess(&balances[i], &balances[j])
	})
	nonces := append([]NonceUpdate(nil), u.Nonces...)
	sort.Slice(nonces, func(i, j int) bool {
		return nonceLess(&nonces[i], &nonces[j])
	})
	codes := append([]CodeUpdate(nil), u.Codes...)
	sort.Slice(codes, func(i, j int) bool {
		return codeLess(&codes[i], &codes[j])
	})
	slots := append([]SlotUpdate(nil), u.Slots...)
	sort.Slice(slots, func(i, j int) bool {
		return slotLess(&slots[i], &slots[j])
	})
...
func UpdateFromBytes(data []byte) (Update, error) {
	...
	if balancesSize > 0 {
		if len(data) < int(balancesSize)*(len(Address{})+amount.BytesLength) {
		...
		for i := 0; i < int(balancesSize); i++ {
			...
			data = data[len(Address{}):]
			res.Balances[i].Balance = amount.NewFromBytes(data[:amount.BytesLength]...)
			data = data[amount.BytesLength:]
		}
	}
