...
func offsetReadValue(reader io.ReadSeeker) (uint64, uint64, error) {
	var buf [16]byte
...
func TestOffsetFile_GetAll_ReturnsAllEntries(t *testing.T) {
	require := require.New(t)
	f, _ := openTestOffsetFile(t)
...
func TestOffsetFile_GetAll_ReturnsLatestValueAfterOverwrite(t *testing.T) {
	require := require.New(t)
	f, _ := openTestOffsetFile(t)
...
func TestOffsetFile_GetAll_ReturnsErrorWhenFileCannotBeOpened(t *testing.T) {
	require := require.New(t)
	f, path := openTestOffsetFile(t)

	require.NoError(f.Set(1, 1))
	// Delete the file to break the subsequent read.
...
func TestOffsetFile_Close_IsNoopAndReturnsNil(t *testing.T) {
	require := require.New(t)
	f, _ := openTestOffsetFile(t)

	require.NoError(f.Close())
	// Subsequent operations must still work because Close is a no-op.
	require.NoError(f.Set(1, 1))
}
...
func TestOffsetFile_Get_PropagatesReadError(t *testing.T) {
	...
	// the initial scan.
	var scanned bool
	readFn := func(r io.ReadSeeker) (uint64, uint64, error) {
		if !scanned {
...
// TestOffsetFile_SetBatch_ReturnsErrorWhenFileCannotBeOpened ensures that
// SetBatch propagates the open error rather than panicking when the target
// path is no longer accessible.
func TestOffsetFile_SetBatch_ReturnsErrorWhenFileCannotBeOpened(t *testing.T) {
	require := require.New(t)
	f, path := openTestOffsetFile(t)

	// Remove the containing directory so OpenFile fails.
	require.NoError(os.RemoveAll(filepath.Dir(path)))

	err := f.SetBatch(map[uint64]uint64{1: 1})
	require.Error(err)
}

// TestOffsetFile_Set_ReturnsErrorWhenFileCannotBeOpened mirrors the SetBatch
// test above for the single-entry Set path.
func TestOffsetFile_Set_ReturnsErrorWhenFileCannotBeOpened(t *testing.T) {
	require := require.New(t)
	f, path := openTestOffsetFile(t)
