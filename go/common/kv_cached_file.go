...
// OpenKVCachedFile opens a KVCachedFile at the given path, reading existing key-value pairs from the file.
// It skips `headerSize` bytes at the beginning of the file.
func OpenKVCachedFile[K comparable, V any](file KVFileWithMemoryFootprint[K, V], headerSize uint64, cacheSize int, flushBufferThreshold int) *KVCachedFile[K, V] {
...
func (c *KVCachedFile[K, V]) Size() (uint64, error) {
	...
	return uniqueKeys + c.file.Size(), nil
}
...
func (c *KVCachedFile[K, V]) GetMemoryFootprint() *MemoryFootprint {
	...
	for k, v := range c.flushBuffer {
		sizeValues += uint(unsafe.Sizeof(k) + unsafe.Sizeof(v))
	}
	c.mutex.Unlock()
	return NewMemoryFootprint(unsafe.Sizeof(*c) + uintptr(sizeValues) + mf.Total() + fileFootprint.Total())
}
