...
func TestCreateAccounts_Success(t *testing.T) {
	state, err := newS6State(state.Parameters{})
	if err != nil {
		t.Fatalf("failed to create verkle state: %v", err)
	}
	defer func() {
	...
	for i := 0; i < numBlocks*numInsertsPerBlock; i++ {
		...
		if !exists {
			t.Errorf("account %s should exist but does not", addr)
		}
