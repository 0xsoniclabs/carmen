...
var (
	...
	emptyCodeHash      = common.GetHash(sha3.NewLegacyKeccak256(), []byte{})
)
...
func (t *VerkleTrie) GetAccount(addr common.Address) (*AccountInfo, error) {
	var (
		acc    = &AccountInfo{}
		values [][]byte
		err    error
	...
	acc.CodeHash = common.Hash(values[utils.CodeHashLeafKey])
...
func (t *VerkleTrie) GetStorage(addr common.Address, key []byte) ([]byte, error) {
	k := utils.StorageSlotKeyWithEvaluatedAddress(t.cache.Get(addr[:]), key)
...
func (t *VerkleTrie) UpdateAccount(addr common.Address, acc *AccountInfo, codeLen int) error {
	var (
		err       error
		basicData [32]byte
...
func (t *VerkleTrie) UpdateStorage(address common.Address, key, value []byte) error {
	...
	k := utils.StorageSlotKeyWithEvaluatedAddress(t.cache.Get(address[:]), key)
...
func (t *VerkleTrie) DeleteAccount(addr common.Address) error {
	k := utils.BasicDataKeyWithEvaluatedAddress(t.cache.Get(addr[:]))
	...
	for i, v := range values {
		...
		case 0:
			prefunded = len(v) == 32
		case 1:
			prefunded = len(v) == 32 && bytes.Equal(v, emptyCodeHash[:])
		default:
			prefunded = v == nil
		}
		if !prefunded {
	...
	if prefunded {
		t.root.Insert(k, (&common.Hash{})[:], t.nodeResolver)
	}
	return nil
}
...
func (t *VerkleTrie) RollBackAccount(addr common.Address) error {
	var (
		evaluatedAddr = t.cache.Get(addr[:])
...
func (t *VerkleTrie) UpdateContractCode(addr common.Address, codeHash common.Hash, code []byte) error {
	...
	for i, chunknr := 0, uint64(0); i < len(chunks); i, chunknr = i+32, chunknr+1 {
		...
		if groupOffset == 0 /* start of new group */ || chunknr == 0 /* first chunk in header group */ {
			values = make([][]byte, verkle.NodeWidth)
			key = utils.CodeChunkKeyWithEvaluatedAddress(t.cache.Get(addr[:]), uint256.NewInt(chunknr))
		}
