...
func TestArchiveTrie_GetCode_ForwardsGetCodeForHashError(t *testing.T) {
	...
	roots, err := loadRoots(t.TempDir())
	if err != nil {
		t.Fatalf("failed to load empty root list: %v", err)
	}
	archive := &ArchiveTrie{head: live, forest: db, roots: roots}
...
func TestArchiveTrie_RootListStoreOnlyWritesNewRoots(t *testing.T) {
	...
	// each Flush.
	ctrl := gomock.NewController(t)
	file := common.NewMockKVFileWithMemoryFootprint[uint64, Root](ctrl)
	...
	cached, err := common.OpenKVCachedFile[uint64, Root](file, 10000, 1000)
	if err != nil {
