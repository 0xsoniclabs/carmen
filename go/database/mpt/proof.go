...
func CreateWitnessProof(nodeSource NodeManager, root *NodeReference, address common.Address, keys ...common.Key) (WitnessProof, error) {
	...
	_, err := visitPathToAccountWithHashAccess(nodeSource, root, address, MakeVisitor(func(node Node, info NodeInfo) VisitResponse {
		...
		if account, ok := node.(*AccountNode); ok {
			for _, key := range keys {
				_, err := visitPathToStorageWithHashAccess(nodeSource, &account.storage, key, visitor)
