...
func TestCheckForest_DetectsIssuesInTrees(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			err := CheckForest(ctxt, func(yield func(uint64, *NodeReference) bool) {
...
func TestCheckForest_AcceptsValidReUse(t *testing.T) {
	...
	if err := CheckForest(ctxt, func(yield func(uint64, *NodeReference) bool) {
		yield(ref1.Id().Index(), &ref1)
		yield(ref2.Id().Index(), &ref2)
	}); err != nil {
		t.Errorf("unexpected error: %v", err)
	}
}
