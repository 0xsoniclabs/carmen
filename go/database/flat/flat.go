...
func NewState(path string, backend state.State) (*State, error) {
	...
	if _, err := os.Stat(file); err == nil {
		...
		err = errors.Join(state.load(file), file.Close())
		if err != nil {
		...
		if err := emptyFile.Close(); err != nil {
			return nil, fmt.Errorf("failed to close empty flat state file: %w", err)
		}
	}
...
func (s *State) flushToDisk() error {
	...
	err = errors.Join(s.store(file), file.Close())
	if err != nil {
		return fmt.Errorf("failed to store flat state to disk: %w", err)
	}
	return nil
}
