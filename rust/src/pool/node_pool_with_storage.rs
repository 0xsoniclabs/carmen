    ...
    S: Storage<Id = NodeId, Item = Node>,
{
    ...
    fn get(
        ...
        let entry = match entry {
            ...
            None => {
                ...
                let entry = Arc::new(RwLock::new(NodeWithMetadata::new(self.storage.get(id)?)));
    ...
    fn add(&self, node: Self::Item) -> Result<Self::Id, Error> {
        ...
        let entry = Arc::new(RwLock::new(NodeWithMetadata {
            value: Node::Empty,
...
mod tests {
    ...
    fn node_pool_with_storage_impl_delete_removes_entry_from_cache_and_storage() {
        let mut storage = MockStorage::new();
        let id = NodeId::from_idx_and_node_type(0, NodeType::Empty);
        let entry = Arc::new(RwLock::new(NodeWithMetadata::new(Node::Empty)));
        storage
            .expect_delete()
            .times(1)
            .with(eq(id))
    ...
    fn node_pool_with_storage_impl_delete_fails_on_storage_error() {
        ...
        cache.cache.insert(
            id,
            Arc::new(RwLock::new(NodeWithMetadata::new(Node::Empty))),
        );
