...
mod tests {
    ...
    fn open_reads_frozen_part_of_file(#[case] mode: FileMode) {
        ...
        let frozen_count = 2;
        let root_ids_file = RootIdsFile::open(path, frozen_count, mode).unwrap();
    ...
    #[rstest_reuse::apply(all_file_modes)]
    fn open_returns_error_for_invalid_file_size(#[case] mode: FileMode) {
        let dir = TestDir::try_new(Permissions::ReadWrite).unwrap();
        let path = dir.join("root_ids");
        ...
        let frozen_count = 2;
        let result = RootIdsFile::open(path, frozen_count, mode);
        assert!(matches!(
    ...
    fn open_fails_if_file_can_not_be_created() {
        ...
        let result = RootIdsFile::open(path, 0, FileMode::ReadWrite);
        assert!(matches!(
            result.map_err(BTError::into_inner),
            Err(Error::Io(_))
        ));
