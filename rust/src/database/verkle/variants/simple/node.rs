...
impl Node {
    ...
    pub fn store(self, updates: &KeyedUpdates, depth: u8) -> Node {
        match self {
            Node::Empty => {
                if depth == 0 {
...
impl InnerNode {
    ...
    ///
    /// Consumes the node and returns an updated version.
    pub fn store(mut self, updates: &KeyedUpdates, depth: u8) -> Node {
...
impl LeafNode {
    ...
    pub fn store(mut self, updates: &KeyedUpdates, depth: u8) -> Node {
        if updates.all_stems_eq(&self.stem) {
...
mod tests {
    ...
    fn empty_node_store_creates_inner_node() {
        ...
        let node = Node::Empty.store(&KeyedUpdates::from_key_value_pairs(&[(key, value)]), 0);
    ...
    fn inner_node_lookup_returns_value_from_next_node() {
        ...
        let root = root.store(
            &KeyedUpdates::from_key_value_pairs(&[(key1, make_value(42))]),
            2,
        );
        let root = root.store(
            &KeyedUpdates::from_key_value_pairs(&[(key2, make_value(84))]),
            2,
        );
    ...
    fn inner_node_store_creates_new_leaf_if_there_is_no_next_node() {
        ...
        let inner = inner.store(
            &KeyedUpdates::from_key_value_pairs(&[(key, make_value(42))]),
            2,
        );
    ...
    fn inner_node_commit_dirty_state_is_tracked() {
        ...
        let inner = inner.store(
            &KeyedUpdates::from_key_value_pairs(&[(make_key(&[1, 2, 4]), make_value(84))]),
            2,
        );
        let Node::Inner(inner) = inner else {
            panic!("expected InnerNode after set");
    ...
    fn leaf_node_can_store_and_lookup_values() {
        ...
        let leaf = leaf.store(
            &KeyedUpdates::from_key_value_pairs(&[(key1, make_value(10))]),
            0,
        );
        let Node::Leaf(leaf) = leaf else {
            panic!("expected LeafNode after set");
        ...
        // used.
        let leaf = leaf.store(
            &KeyedUpdates::from_key_value_pairs(&[(key2, Value::default())]),
            0,
        );
        let Node::Leaf(leaf) = leaf else {
            panic!("expected LeafNode after set");
        ...
        let leaf = leaf.store(
            &KeyedUpdates::from_key_value_pairs(&[(key1, Value::default())]),
            0,
        );
        let Node::Leaf(leaf) = leaf else {
            panic!("expected LeafNode after set");
    ...
    fn leaf_node_can_compute_commitment() {
        ...
        let leaf = LeafNode::new(&key1);
        let leaf = leaf.store(&KeyedUpdates::from_key_value_pairs(&[(key1, val1)]), 0);
        let leaf = leaf.store(&KeyedUpdates::from_key_value_pairs(&[(key2, val2)]), 0);
    ...
    fn leaf_node_commitment_dirty_state_is_tracked() {
        ...
        let leaf = leaf.store(
            &KeyedUpdates::from_key_value_pairs(&[(key1, make_value(30))]),
            0,
        );
        let Node::Leaf(mut leaf) = leaf else {
            panic!("expected LeafNode after set");
