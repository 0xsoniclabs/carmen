...
mod tests {
    use super::*;
    use crate::{
        database::verkle::{test_utils::FromIndexValues, variants::managed::nodes::NodeType},
        error::BTError,
        types::TreeId,
    };
    ...
    fn next_store_action_at_depth_zero_transforms_to_inner_node() {
        ...
        let key = Key::default();
        let action = node
            .next_store_action(&key, 0, NodeId::from_idx_and_node_type(0, NodeType::Empty))
            .unwrap();
        match action {
            StoreAction::HandleTransform(Node::Inner(_)) => {}
    ...
    fn next_store_action_at_depth_above_zero_transforms_to_leaf_node() {
        ...
        let action = node
            .next_store_action(&key, 1, NodeId::from_idx_and_node_type(0, NodeType::Empty))
            .unwrap();
        match action {
            StoreAction::HandleTransform(leaf) => {
                ...
                assert_eq!(
                    leaf.next_store_action(
                        &key,
                        0,
                        NodeId::from_idx_and_node_type(0, NodeType::Empty) // type does not matter
                    ),
