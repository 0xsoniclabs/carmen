...
mod tests {
    ...
    trait VerkleManagedTrieNode:
        ManagedTrieNode<Union = Node, Id = NodeId, Commitment = VerkleCommitment>
        + GenericHelperTrait
    {
    }
    ...
    fn make_node_id() -> NodeId {
        NodeId::from_idx_and_node_type(123, NodeType::Leaf2) // The actual NodeType is irrelevant for tests
    }
    ...
    fn from_existing_copies_stem_and_values_and_commitment_correctly() {
        ...
        // Note that the search internally first advances to index 18 % 3, which happens to be 0.
        ...
        let node = SparseLeafNode::<3>::from_existing(STEM, &values, commitment).unwrap();
        // Since the first slot is taken by index 18, index 0 and 1 get shifted back by one slot.
        assert_eq!(node.values[0], values[0]);
    ...
    fn get_slot_returns_slot_with_matching_index_or_empty_slot() {
        ...
        // Matching index
        let slot = SparseLeafNode::<7>::get_slot_for(&node.values, INDEX1);
        assert_eq!(slot, Some(0));

        // Matching index has precedence over empty slot
        node.values[1].value = Value::default();
        let slot = SparseLeafNode::<7>::get_slot_for(&node.values, INDEX1 + 3);
    ...
    #[rstest_reuse::apply(different_leaf_sizes)]
    fn store_sets_value_at_final_key_position(#[case] node: Box<dyn VerkleManagedTrieNode>) {
        let mut node = node;
        let position = 78;
