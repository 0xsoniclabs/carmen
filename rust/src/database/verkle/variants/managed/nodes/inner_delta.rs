        ...
        verkle::{
            ...
            variants::managed::{
                ...
                nodes::{
                    ...
                    id::VerkleNodeId, inner::OnDiskInnerNode,
                },
            },
        },
        visitor::NodeVisitor,
...
#[repr(C)]
pub struct OnDiskInnerDeltaNode {
    pub children: [VerkleIdWithIndex; InnerDeltaNode::DELTA_SIZE],
    pub full_inner_node_id: VerkleNodeId,
    pub commitment: OnDiskVerkleCommitment,
}

// this still needs to lead the full inner node to reconstruct
impl From<OnDiskInnerDeltaNode> for InnerDeltaNode {
    fn from(delta_node: OnDiskInnerDeltaNode) -> Self {
        InnerDeltaNode {
            ...
            children_delta: delta_node.children,
...
impl From<(OnDiskInnerDeltaNode, OnDiskInnerNode)> for InnerDeltaNode {
    fn from((delta_node, full_node): (OnDiskInnerDeltaNode, OnDiskInnerNode)) -> Self {
        InnerDeltaNode {
            children: full_node.children,
            children_delta: delta_node.children,
            full_inner_node_id: delta_node.full_inner_node_id,
            commitment: VerkleCommitment::from(delta_node.commitment),
        }
...
// impl Default for InnerDeltaNode {
//     fn default() -> Self {
...
impl ManagedTrieNode for InnerDeltaNode {
    ...
    fn replace_child(&mut self, key: &Key, depth: u8, new: VerkleNodeId) -> BTResult<(), Error> {
        // TODO what happens if we want to replace a child that is not in the delta?
...
impl ManagedInnerNode for InnerDeltaNode {
