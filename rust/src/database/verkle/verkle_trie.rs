...
use crate::{
    database::verkle::{crypto::Commitment, keyed_update::KeyedUpdates},
...
mod tests {
    ...
    fn values_can_be_updated(#[case] trie: Box<dyn VerkleTrie>) {
        ...
        trie.store(&KeyedUpdates::from_key_value_pairs(&[(key, make_value(1))]))
            .unwrap();
        assert_eq!(trie.lookup(&key).unwrap(), make_value(1));
        trie.store(&KeyedUpdates::from_key_value_pairs(&[(key, make_value(2))]))
            .unwrap();
        assert_eq!(trie.lookup(&key).unwrap(), make_value(2));
        trie.store(&KeyedUpdates::from_key_value_pairs(&[(key, make_value(3))]))
            .unwrap();
