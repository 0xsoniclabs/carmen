...
impl VerkleTrieEmbedding {
    ...
    /// Returns the key of the basic data fields (nonce, balance, code size) for the given address.
    pub fn try_get_basic_data_key(&self, address: &Address) -> Option<Key> {
        self.try_get_trie_key(address, &U256::ZERO, 0)
    }
    ...
    /// Returns the key of the code hash field for the given address.
    pub fn try_get_code_hash_key(&self, address: &Address) -> Option<Key> {
        self.try_get_trie_key(address, &U256::ZERO, 1)
    }
    ...
    /// Returns the key of the code chunk with the given chunk number for the given address.
    pub fn try_get_code_chunk_key(&self, address: &Address, chunk_number: u32) -> Option<Key> {
        // Derived from
        // https://github.com/0xsoniclabs/go-ethereum/blob/e563918a84b4104e44935ddc6850f11738dcc3f5/trie/utils/verkle.go#L188
        ...
        let (tree_index, sub_index) = chunk_offset.div_rem(&VERKLE_NODE_WIDTH.to_nz().unwrap());
        let sub_index = sub_index.to_words()[0] as u8;
        self.try_get_trie_key(address, &tree_index, sub_index)
    }
    ...
    /// Returns the storage key for the given address and storage key.
    pub fn try_get_storage_key(&self, address: &Address, key: &Key) -> Option<Key> {
        // Derived from
        // https://github.com/0xsoniclabs/go-ethereum/blob/e563918a84b4104e44935ddc6850f11738dcc3f5/trie/utils/verkle.go#L203
        ...
        self.try_get_trie_key(address, &tree_index, suffix)
    }
    ...
    /// Retrieves the Verkle trie key for the given address, tree index, and sub index from a cache
    /// or returns None if not found.
    fn try_get_trie_key(&self, address: &Address, tree_index: &U256, sub_index: u8) -> Option<Key> {
        self.cache.get(&(*address, *tree_index)).map(|mut k| {
            k[31] = sub_index;
            k
        })
    }
