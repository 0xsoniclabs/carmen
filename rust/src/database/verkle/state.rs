...
impl<T: VerkleTrie> CarmenState for VerkleTrieCarmenState<T> {
    ...
    fn apply_block_update<'u>(&self, _block: u64, update: Update<'u>) -> BTResult<(), Error> {
        for addr in update.created_accounts.iter() {
        ...
        for u in update.nonces.iter() {
            println!("Updating nonce for addr {:?} to {:?}", u.addr, u.nonce);
        ...
        for u in update.balances.iter() {
            println!("Updating balance for addr {:?} to {:?}", u.addr, u.balance);
            let key = get_basic_data_key(&u.addr);
            let mut value = self.trie.lookup(&key)?;
        ...
        for u in update.codes {
            println!("Updating code for addr {:?}, len {}", u.addr, u.code.len());
            // Store code length
        ...
        for u in update.slots.iter() {
            println!("Updating slot for addr {:?}, key {:?}", u.addr, u.key);
...
//     #[rstest::rstest]
//     #[case::simple_in_memory(Box::new(VerkleTrieCarmenState::<SimpleInMemoryVerkleTrie>::new())
// as Box<dyn CarmenState>)]     fn all_state_impls(#[case] state: Box<dyn CarmenState>) {}
...
//     #[rstest_reuse::apply(all_state_impls)]
//     fn creating_account_does_not_overwrite_basic_account_data(#[case] state: Box<dyn
// CarmenState>) {         let addr = Address::from_index_values(0, &[(0, 1)]);
