    ...
    while !current_node_updates.is_empty() {
        ...
        while let Some(current_node_update) = current_node_updates.get_mut(i) {
            ...
            match current_node.next_store_action(
                // updates is a Cow::Borrowed so the clone is cheap
                ...
                StoreAction::Descend(descent_actions) => {
                    ...
                    for DescendAction { id, updates } in descent_actions {
                        ...
                        let node = manager.get_write_access(id)?;
                        if matches!(
                            node.next_store_action(updates.clone(), depth + 1, id)?,
                            StoreAction::HandleReparent(_) | StoreAction::HandleTransform(_)
                        ) {
                        ...
                        next_node_updates.push(DescendUpdates {
                            parent_index: Some(i),
                            node: if id.is_empty_id() { None } else { Some(node) },
                            is_new: false,
                            node_id: id,
                            updates,
                        });
