...
grouped_df = df.groupby('Node Type').apply(lambda x: x.assign(
    PrefixSum=x['Node Count'].cumsum())).reset_index(drop=True)
...
def sparse_inner_node_size(
    num_children): return commitment_size + num_children * (id_size + id_index)
...
def solve_mip(num_nodes_to_use, node_count_by_type: dict, node_prefix_sum: dict, max_node: int, node_sizes: dict, greedy_solution: dict, node_pruning_threshold: float):
    ...
    nodes_lp_variable = {
        i: LpVariable(f"n_{i}", lowBound=0,
                      upBound=node_prefix_sum[i], cat=LpInteger)
    ...
    for i in nodes_range:
        problem += nodes_lp_variable[i] <= node_prefix_sum[i] - lpSum(nodes_lp_variable[j] for j in range(
    ...
    problem += lpSum([node_exist_binary[i] for i in nodes_range]
...
def plot_solution_sizes(node_name, node_range, solution_sizes_greedy, solution_sizes_mip):
    def _plot(node_name, node_range, solution_sizes_greedy, solution_sizes_mip):
        ...
        plt.xlabel('Number of specialization used')
    ...
    _plot(node_name + " (without size 1)", node_range,
