...
use crate::{
    error::Error,
    node_pool::{NodePool, NodePoolEntry},
    storage::Storage,
    types::{Node, NodeId},
};
...
/// A thread-safe [`NodePool`], which uses a [`Storage`] backend to persist and retrieve entries.
...
/// - An entry will never be evicted as long as a reference to it exists outside of the pool
#[derive(Clone)]
...
/// The internal implementation of the [`NodePoolWithStorage`], which implements the [`NodePool`]
/// trait.
...
impl<S> NodePool<NodeWithMetadata> for NodePoolWithStorageImpl<S>
where
    S: Storage<Id = NodeId, Item = Node>,
{
    ...
    fn get(&self, id: NodeId) -> Result<NodePoolEntry<NodeWithMetadata>, Error> {
        ...
        Ok(NodePoolEntry::new(entry))
    }

    /// Stores the node in the pool and reserves a [`NodeId`] for it.
    fn set(&self, node: Node) -> Result<NodeId, Error> {
...
mod tests {
    ...
    fn node_pool_impl_get_returns_existing_entry_from_storage_if_not_in_cache() {
        ...
        assert!(entry.read().unwrap().value == expected_entry);
    }
    ...
    fn node_pool_impl_get_always_insert_element_in_cache() {
        ...
        for i in 0..NUM_ELEMENTS {
            ...
            assert!(entry.write().unwrap().status == NodeStatus::Dirty);
    ...
    fn node_pool_impl_set_inserts_elements_in_cache() {
        ...
        let entry = cache.get(id).expect("get should succeed");
        assert!(entry.read().unwrap().value == Node::Empty);
    }
