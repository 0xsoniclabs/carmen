...
pub enum Caches {
    QuickCache(quick_cache::sync::Cache<u64, u64, UnitWeighter, RandomState, RandomLifecycle>),
    CachedNodeManager(CachedNodeManager<ProducerStorage>),
    LockCache(lock_cache::LockCache<u64, u64>),
}

/// Different cache types to benchmark
...
impl Caches {
        ...
        for i in 0..self.capacity() {
            match self {
                Caches::QuickCache(cache) => {
                    cache.insert(i, 42u64);
                }
                Caches::LockCache(lock_cache) => {
                    let _unused = lock_cache
                        .get_read_access_or_insert(i, || Ok(42u64))
                        .unwrap();
                }
    ...
    pub fn execute_read_op(&self, iter: u64) -> BTResult<(), Error> {
        match self {
            Caches::QuickCache(cache) => match cache.get_value_or_guard(&iter, None) {
                quick_cache::sync::GuardResult::Guard(guard) => {
                    let _ = guard.insert(42u64);
                    Ok(())
                }
                quick_cache::sync::GuardResult::Value(_) => Ok(()),
            ...
            Caches::LockCache(lock_cache) => {
                let _node = lock_cache
                    .get_read_access_or_insert(iter, || Ok(42u64))
                    .unwrap();
                Ok(())
            }
        }
...
/// Read data from the cache, varying cache size, cache hit or miss and number of threads used.
        ...
        for in_cache in [true, false] {
            ...
            for num_threads in pow_2_threads() {
                let mut bench_group = c.benchmark_group(format!(
                    "caching/read/CacheSize:{cache_size}/InCache:{in_cache}/NumThreads:{num_threads}"
                ));
                for cache_type in [
                            ...
                            b.iter_custom(|iters| {
                                execute_with_threads(
                                    *num_threads as u64,
                                    ...
                                    || (),
                                    |iter, _| cache.execute_read_op(iter),
                                )
                            });
                        },
                    );
...
/// Benchmark that tests the effect of different pinning probabilities on cache performance
        ...
        for cache_size in [100_000, 1_000_000] {
            for num_threads in pow_2_threads() {
                for cache_types in [
                    ...
                    let mut cache = cache_types.make_cache(cache_size, pinning_prob);
                    cache.fill();
                    let mut bench_group = c.benchmark_group(format!(
                        "caching/pinning/{cache_types}/CacheSize:{cache_size}/PinningProb:{pinning_prob}"
                    ));
                    let mut completed_iterations = 0u64;
                    bench_group.bench_with_input(
                        BenchmarkId::from_parameter(format!("{num_threads}")),
