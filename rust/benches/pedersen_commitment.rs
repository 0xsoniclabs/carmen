...
fn commitment_benchmark(c: &mut Criterion) {
    ...
    // We choose indices 0 and 4 because these are still within the more aggressively optimized
    // range of the [`banderwagon::DefaultCommitter`]. The other indices are not.
    let scenarios = [
    ...
    for (name, index_values) in scenarios {
        ...
        g.bench_with_input(
            BenchmarkId::from_parameter("update new"),
            &index_values,
            |b, index_values| {
                b.iter(|| {
