...
fn execute(
    ...
    group.bench_with_input(
        BenchmarkId::from_parameter(format!("{pinning_prob}pinning_prob/{dirty_nodes_proportions}dirty_nodes_prop/{num_threads}threads")),
        &(),
        |b, _| {
        ...
        for manager_size in [100_000] {
            ...
            for node_proportions in &node_proportions_vec {
                for operations_proportions in &operations_proportions_vec {
                    let mut group = c
                            .benchmark_group(format!("random_ops/{storage_type:?}/{manager_size}/{node_proportions:?}/{operations_proportions:?}"));
                    for pinning_prob in [10, 25] {
